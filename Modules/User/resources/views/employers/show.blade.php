...
<div class="mx-auto max-w-[1120px] px-4 py-8">
    <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div class="grid lg:grid-cols-[0.9fr,1.1fr]">
            <div class="bg-gradient-to-br from-[#eef6ff] via-white to-[#f8fbff] p-8">
                <div class="flex min-h-48 items-center justify-center rounded-3xl border border-[#d8e8f8] bg-white p-8 shadow-sm">
                    @if($isBmoEmployer)
                        <img src="{{ asset('images/employers/bmo.svg') }}" alt="BMO" class="h-24 w-auto max-w-full">
                    @else
    ...
    <section class="mt-8">
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            @forelse($listings as $listing)
                <a href="{{ route('listings.show', $listing) }}" class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                    <p class="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">{{ $listing->category?->name ?? 'Job' }}</p>
                    <h3 class="mt-3 text-lg font-bold leading-tight text-slate-950">{{ $listing->title }}</h3>
                    <p class="mt-3 text-sm font-semibold text-[#005eb8]">{{ $formatCompensation($listing) }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ collect([$listing->city, $listing->country])->filter()->join(', ') }}</p>
                </a>
            @empty
